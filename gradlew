...
then
    ...
    # Assuming that Authoring generates a git remote of the form https://<user>:<token>@<hostname>:<port>/<path-to-repo>
    ...
    # If not set, default to the URIs inferred from the git remote
    export ORG_GRADLE_PROJECT_transformsMavenProxyRepoUri="https://${git_host_and_port}/artifacts/api/legacy/mrp/authz/all"
    export ORG_GRADLE_PROJECT_artifactsUri="https://${git_host_and_port}/artifacts/api"
    export ORG_GRADLE_PROJECT_transformsBearerToken="${git_password}"

    wrapperAuthGradleOptions="-Dgradle.wrapperUser=${git_username} -Dgradle.wrapperPassword=${git_password}"
    export GRADLE_OPTS="${GRADLE_OPTS:-} ${wrapperAuthGradleOptions}"

    transformsGradleDistributionUrl="${ORG_GRADLE_PROJECT_artifactsUri}/repositories/ri.gradle.distributions.artifacts.repository/contents/release/files/gradle-5.4.1-bin.zip"
fi
